...
class MeshDeviceReportTest(unittest.TestCase):
    ...
    def test_accepts_partition_leader_but_rejects_inconsistent_role_flags(self) -> None:
        """A regular pole may lead a partition, but its reported flags must agree."""
        expected_release = "DIG2GO_TUBES"
        ...
        inconsistent_follower = REPORTS.parse_report_line(valid_line.replace("uplink=3850", "uplink=0"))
        assert leading is not None
        assert forced_master is not None
        assert inconsistent_follower is not None
        ...
        self.assertIsNone(REPORTS.report_mismatch(leading, *expected))
        self.assertEqual(
            REPORTS.report_mismatch(forced_master, *expected),
            "master behavior is True, but stored role 10 expects False",
        )
        self.assertEqual(
            REPORTS.report_mismatch(inconsistent_follower, *expected),
            "mesh follower flag and Control uplink disagree",
        )
    # AI: end
