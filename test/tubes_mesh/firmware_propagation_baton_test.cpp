...
FirmwareUpdateCompletionProof completedProof(uint32_t sessionNonce = 0x1234) {
  ...
  const FirmwareTargetContract target = exactTarget();
  expect(session.select(SENDER, RECEIVER, artifact, target,
      firmwareInactiveSlotDestination(target), 0, 100, sessionNonce),
      "session select failed");
  expect(session.startTransfer(RECEIVER, 1), "session transfer failed");
  expect(session.recordProgress(RECEIVER, artifact.imageLengthBytes, 2),
      "session progress failed");
  expect(session.verifyTransfer(RECEIVER, artifact.imageSha256, 3),
      "session hash failed");
  FirmwareUpdateHealthProof proof;
  proof.target = target;
  proof.releaseHash = artifact.releaseHash;
  memcpy(proof.imageSha256, artifact.imageSha256, sizeof(proof.imageSha256));
  proof.runtimeConfigurationPreserved = true;
  proof.meshRejoined = true;
  proof.stable = true;
  expect(session.proveHealthy(RECEIVER, proof, 4) && session.complete(RECEIVER),
...
void health_failure_never_hands_off_the_baton() {
  ...
  expect(baton.start(SENDER, exactArtifact(), 0), "seed did not start");
  expect(baton.selectReceiver(RECEIVER, FirmwarePeerTargetable, 0x1234, 1), "receiver was rejected");
  expect(baton.recordProgress(RECEIVER, exactArtifact().imageLengthBytes), "exact progress was rejected");
...
void activity_refreshes_deadline_in_every_active_state_and_wraps_safely() {
  FirmwarePropagationBaton baton;
  expect(baton.start(SENDER, exactArtifact(), 0xfffffff0U), "seed did not start");
  expect(baton.refreshActivity(0xfffffff5U), "looking activity was rejected");
  expect(!baton.expireIfIdle(0x00000030U), "looking state expired after refreshed activity");
  expect(baton.selectReceiver(RECEIVER, FirmwarePeerTargetable, 0x1234, 0x00000020U), "receiver was rejected");
  expect(baton.refreshActivity(0x00000021U), "transferring activity was rejected");
  expect(!baton.expireIfIdle(0x0000005bU), "transferring state expired after refreshed activity");
  expect(baton.recordProgress(RECEIVER, exactArtifact().imageLengthBytes), "exact progress was rejected");
  expect(baton.awaitHealth(RECEIVER), "health wait was rejected");
  expect(baton.refreshActivity(0x0000006eU), "health activity was rejected");
  expect(!baton.expireIfIdle(0x000000a9U), "health state expired after refreshed activity");
  expect(baton.expireIfIdle(0x000000aaU), "health state did not expire at refreshed deadline");
}

void progress_cannot_exceed_frozen_artifact_or_enter_health_early() {
  FirmwarePropagationBaton baton;
  expect(baton.start(SENDER, exactArtifact(), 0), "seed did not start");
  expect(baton.selectReceiver(RECEIVER, FirmwarePeerTargetable, 0x1234, 1), "receiver was rejected");
  const size_t length = exactArtifact().imageLengthBytes;
  expect(!baton.recordProgress(RECEIVER, length + 1), "oversized progress was accepted");
  expect(!baton.awaitHealth(RECEIVER), "early health wait was accepted");
  expect(baton.recordProgress(RECEIVER, length), "exact completion progress was rejected");
  expect(baton.awaitHealth(RECEIVER), "exact completion did not permit health wait");
}

void retry_semantics_are_clear_and_bounded() {
  FirmwarePropagationBaton baton;
  expect(baton.start(SENDER, exactArtifact(), 0), "seed did not start");
  expect(baton.selectReceiver(RECEIVER, FirmwarePeerTargetable, 0x1234, 1), "receiver was rejected");
  expect(!baton.retryFromZero(OTHER), "wrong receiver used retry budget");
  expect(baton.retryFromZero(RECEIVER), "first retry was rejected");
  expect(!baton.retryFromZero(RECEIVER), "second retry was accepted");
}
...
int main() {
  const std::array<std::pair<const char*, void (*)()>, 9> tests = {{
    ...
    {"activity refreshes deadline in every active state and wraps safely", activity_refreshes_deadline_in_every_active_state_and_wraps_safely},
    ...
    {"retry semantics are clear and bounded", retry_semantics_are_clear_and_bounded},
