...
constexpr DeviceId ROOT_ID = 300;
constexpr DeviceId RELAY_ID = 200;
constexpr DeviceId LEAF_ID = 100;
...
// A v2 packet crossing a v3-capable relay keeps outer version 2, because changing
// only the version byte would make downstream devices select the wrong decoder.
void relay_preserves_the_received_protocol_version() {
    MeshNodeHeader relayHeader;
    relayHeader.id = RELAY_ID;
    relayHeader.uplinkId = ROOT_ID;
    relayHeader.version = TUBES_PROTOCOL_V3;
    NodeMessage legacy;
    legacy.header.id = LEAF_ID;
    legacy.header.version = TUBES_PROTOCOL_V2;
    legacy.recipients = RECIPIENTS_ALL;
    legacy.command = TEST_BEAT_COMMAND;

    NodeMessage relayed = makeRelayedMessage(relayHeader, true, legacy);

    expect(relayed.header.version == TUBES_PROTOCOL_V2, "relay changed a v2 packet into a v3 packet");
    expect(relayed.header.id == RELAY_ID, "relay did not stamp its physical sender ID");
}
// AI: end
...
int main() {
    const std::array<std::pair<const char*, void (*)()>, 5> tests = {{
        ...
        {"relay preserves the received protocol version", relay_preserves_the_received_protocol_version},
    }};
