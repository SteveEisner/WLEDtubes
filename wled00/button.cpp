...
#define WLED_LONG_POWER_SAVE       1000 // how long button 0 needs to be held to activate usermod power-save behavior
...
void shortPressAction(uint8_t b)
{
  if (!buttons[b].macroButton) {
    switch (b) {
      case 0: toggleOnOff(); UsermodManager::handleButton(101); stateUpdated(CALL_MODE_BUTTON); break;
    ...
    if (isButtonPressed(b)) { // pressed
      ...
      if (b == 0 && dur > WLED_LONG_POWER_SAVE) { // long press on button 0 (when released)
        if (dur > WLED_LONG_FACTORY_RESET) { // factory reset if pressed > 10 seconds
          ...
          UsermodManager::handleButton(100);
        }
        ...
        if (b > 1 && !buttons[b].macroDoublePress) { //don't wait for double press on buttons without a default action if no double press macro set
          ...
          if (doublePress) {
            doublePressAction(b);
            if (b == 0) UsermodManager::handleButton(102);
          } else {
