...
// Inspired by BusNetwork's bounded framebuffer allocation and pixel access in
...
void BusVirtualFramebuffer::setPixelColor(unsigned pix, uint32_t c) {
  if (!_valid || pix >= _len) return;
  if (_reversed) pix = _len - pix - 1;
  _data[pix] = c;
}

uint32_t BusVirtualFramebuffer::getPixelColor(unsigned pix) const {
  if (!_valid || pix >= _len) return 0;
  if (_reversed) pix = _len - pix - 1;
  return _data[pix];
}
...
#ifdef TUBES_NULL_OUTPUT
  if (type == TYPE_VIRTUAL_FRAMEBUFFER_RGB) {
    mem += sizeof(BusVirtualFramebuffer) + count * sizeof(uint32_t);
  } else
#endif
...
int BusManager::add(const BusConfig &bc, bool placeholder) {
#ifdef TUBES_NULL_OUTPUT
  // The read-only S3 target has no output GPIO or network transport. Reject
  // incompatible persisted configs before pin allocation or bus mutation.
  ...
  if (!busses.empty()) {
    const Bus *existing = busses.front().get();
    if (busses.size() == 1 && existing != nullptr && existing->isOk() &&
        existing->getType() == TYPE_VIRTUAL_FRAMEBUFFER_RGB &&
        existing->getStart() == bc.start && existing->getLength() == bc.count) {
      return busses.size();
    }
    errorFlag = ERR_DENIED;
    DEBUGBUS_PRINTLN(F("Rejected additional framebuffer bus on logical-output target."));
    return -1;
  }
#endif
...
#ifdef TUBES_NULL_OUTPUT
  } else if (bc.type == TYPE_VIRTUAL_FRAMEBUFFER_RGB) {
    auto framebuffer = make_unique<BusVirtualFramebuffer>(bc);
    if (!framebuffer->isOk()) {
      errorFlag = ERR_NORAM_PX;
      DEBUGBUS_PRINTLN(F("Unable to allocate virtual framebuffer bus."));
      return -1;
    }
    busses.push_back(std::move(framebuffer));
#endif
