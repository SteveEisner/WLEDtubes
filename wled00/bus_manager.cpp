    ...
    if (_gMilliAmpsMax > 0) {
      ...
      if (globalMax > totalLEDs) { // check if budget is larger than standby current
        if (milliAmpsSum > globalMax) {
          ...
          // Both values include LED standby current, which cannot be reduced by
          // scaling color. Limit only the color-producing portion of the load.
          uint32_t colorDemand = milliAmpsSum - totalLEDs;
          uint32_t colorBudget = globalMax - totalLEDs;
          newBri = colorBudget * 255 / colorDemand;
          if (!newBri) newBri = 1;
          // AI: end
