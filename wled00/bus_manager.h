...
// Generic logical RGB output. Its framebuffer contract is modeled after WLED's
// BusNetwork storage, without inheriting any network transport behavior.
class BusVirtualFramebuffer : public Bus {
  public:
    BusVirtualFramebuffer(const BusConfig &bc);
    ~BusVirtualFramebuffer();
    void show() override {}
    void setPixelColor(unsigned pix, uint32_t c) override;
    uint32_t getPixelColor(unsigned pix) const override;
    size_t getBusSize() const override { return sizeof(BusVirtualFramebuffer) + (isOk() ? _len * sizeof(uint32_t) : 0); }
