...
#include "wled.h"
#include "relay_startup_policy.h"
...
#if defined(TUBES_DIG2GO_RELAY_STARTUP_POLICY)
  // Dig2Go retains relay configuration across app-only updates. Avoid the
  // generic forced off->on sequence, which causes relay inrush during boot.
  const RelayStartupDecision relay = dig2goRelayStartup(rlyPin >= 0, turnOnAtBoot, briS, rlyMde);
  if (relay.relayPresent) {
    pinMode(rlyPin, rlyOpenDrain ? OUTPUT_OPEN_DRAIN : OUTPUT);
    digitalWrite(rlyPin, relay.outputLevel);
  }
  offMode = relay.offMode;
#else
  // Preserve the existing WLED startup path for every other target.
  if (rlyPin >= 0) {
    pinMode(rlyPin, rlyOpenDrain ? OUTPUT_OPEN_DRAIN : OUTPUT);
    digitalWrite(rlyPin, rlyMde ? bri > 0 : bri == 0);
  }
  offMode = bri == 0;
#endif
}
...
#ifndef WLED_DISABLE_ESPNOW
  if (enableESPNow && !_temporaryStaLeaseActive) {
...
bool WLED::beginTemporaryStaLease(const char* ssid, const char* pass)
{
  if (_temporaryStaLeaseActive || !ssid || !ssid[0] || !pass || multiWiFi.size() >= 15)
    return false;

  _temporaryStaSavedSelection = selectedWiFi;
  _temporaryStaSavedForceReconnect = forceReconnect;
  _temporaryStaSavedInterfacesInited = interfacesInited;
  _temporaryStaSavedWasConnected = wasConnected;
  _temporaryStaSavedLastReconnectAttempt = lastReconnectAttempt;

  multiWiFi.push_back(WiFiConfig(ssid, pass, 0, 0));
  _temporaryStaLeaseIndex = multiWiFi.size() - 1;
  selectedWiFi = _temporaryStaLeaseIndex;
  _temporaryStaLeaseActive = true;
  forceReconnect = false;
  interfacesInited = false;
  wasConnected = false;
  DEBUG_PRINTF_P(PSTR("Temporary STA lease started: profile %u, SSID %s.\n"),
      _temporaryStaLeaseIndex, ssid);
  initConnection();
  return true;
}

bool WLED::endTemporaryStaLease()
{
  if (!_temporaryStaLeaseActive) return true;
  if (multiWiFi.empty() || _temporaryStaLeaseIndex != multiWiFi.size() - 1)
    return false;

  WiFi.disconnect(false, true);
  multiWiFi.pop_back();
  selectedWiFi = _temporaryStaSavedSelection < multiWiFi.size()
      ? _temporaryStaSavedSelection : 0;
  _temporaryStaLeaseActive = false;
  forceReconnect = _temporaryStaSavedForceReconnect;
  interfacesInited = _temporaryStaSavedInterfacesInited;
  wasConnected = _temporaryStaSavedWasConnected;
  lastReconnectAttempt = _temporaryStaSavedLastReconnectAttempt;
  DEBUG_PRINTF_P(PSTR("Temporary STA lease ended; restored profile %d.\n"), selectedWiFi);
  initConnection();
  return true;
}
...
{
  // A temporary station lease owns all connection lifecycle decisions. Normal
  // scans, fallback AP creation, profile rotation, and ESP-NOW loops resume
  // only after the lease is explicitly ended.
  if (_temporaryStaLeaseActive) return;
