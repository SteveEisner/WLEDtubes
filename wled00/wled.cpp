...
// Report configured digital inputs that remain active after configuration.
static void diagnoseBootButtons()
{
  ...
  for (size_t b = 0; b < buttons.size(); b++) {
    ...
    if (button.pin < 0 || button.type == BTN_TYPE_NONE || button.type == BTN_TYPE_ANALOG ||
        button.type == BTN_TYPE_ANALOG_INVERTED || button.type == BTN_TYPE_TOUCH ||
        button.type == BTN_TYPE_TOUCH_SWITCH || button.type == BTN_TYPE_SWITCH ||
        button.type == BTN_TYPE_PIR_SENSOR) continue;
    const bool active = (button.type == BTN_TYPE_PUSH_ACT_HIGH) ?
                        (digitalRead(button.pin) == HIGH) : (digitalRead(button.pin) == LOW);
    if (active) {
      stuck = true;
      Serial.printf("WLED button diagnostic: active at boot index=%u gpio=%d type=%u\\n",
                    (unsigned)b, button.pin, (unsigned)button.type);
    }
  }
  Serial.println(stuck ? F("WLED button diagnostics: STUCK_BUTTON") :
                         F("WLED button diagnostics: healthy"));
#endif
}
// AI: end
...
void WLED::setup()
{
  ...
  if (!wledRescueMode) diagnoseBootButtons();
  DEBUG_PRINTF_P(PSTR("heap %u\\n"), getFreeHeapSize());
