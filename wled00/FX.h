...
class Segment {
  ...
  public:
    ...
    // Copies any WLED palette through the same loader used by the render pipeline.
    inline void copyPalette(CRGBPalette16 &target, uint8_t paletteId) { loadPalette(target, paletteId); }

    // Restores this segment's shared draw state after another segment rendered last.
    inline void restoreDrawState() { updateTransitionProgress(); beginDraw(progress()); }
    // AI: end
