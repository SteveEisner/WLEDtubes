...
class TubesUsermod : public Usermod {
  ...
  public:
    ...
    void handleOverlayDraw() {
      ...
      // Temporary migration marker: generation-1 poles reserve ten yellow pixels.
      if (CURRENT_PROTOCOL_GENERATION == CHANNEL_PROTOCOL_GENERATION) {
        uint16_t markerLength = min(strip.getLengthTotal(), uint16_t(10));
        for (uint16_t i = 0; i < markerLength; i++)
          strip.setPixelColor(i, CRGB::Yellow);
      }
      // AI: end
    }
