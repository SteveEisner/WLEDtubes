...
static constexpr uint8_t S3_BEAT_OVERLAY_COUNT = 8;
static_assert(gSoundProgramCount >= S3_BEAT_OVERLAY_COUNT,
              "TubeOS beat overlay choices require eight sound programs");
// AI: end
...
class PatternController : public MessageReceiver {
  public:
    ...
    PaletteRawDefinition nextPaletteRaw;
    PaletteRawDefinition activePaletteRaw;
    bool activePalette16Valid = false;
    bool nextPalette16Valid = false;
    bool activePaletteRawValid = false;
    bool nextPaletteRawValid = false;
    uint8_t activePaletteSourceId = PALETTE_PREDEFINED_ID_NONE;
    uint8_t nextPaletteSourceId = PALETTE_PREDEFINED_ID_NONE;
    ...
    uint8_t currentSoundProgramChance = SOUND_CHANCE_ALL;
    uint8_t nextSoundProgramChance = SOUND_CHANCE_ALL;
    ...
    UpdateWorkflowStatus dig2GoPeerPropagationOverlayStatus = Idle;
    bool fleetPropagationTransportSuspended = false;
    bool fleetPropagationRestoreStarted = false;
#endif
...
#if TUBES_ENABLE_DIG2GO_PEER_PROPAGATION
  // AI: below section was generated by an AI
  void setDig2GoPropagationCallback(Dig2GoPropagationCallback propagationCallback) {
    dig2GoPropagationCallback = propagationCallback;
  }

  void setDig2GoPeerPropagationOverlay(UpdateWorkflowStatus status) {
    dig2GoPeerPropagationOverlayStatus = status;
  }
  ...
  void setup()
  {
    ...
    // AI: below section was generated by an AI
#if defined(TUBES_S3_FIELD_OS)
    node.reset(makeDeviceId(CURRENT_PROTOCOL_GENERATION, PROTOCOL_LOCAL_VALUE_MASK));
#elif defined(GOLDEN) || defined(CHRISTMAS) || defined(RUBY) || defined(MAUVE) || defined(MASTER)
    node.reset(0xFFF);
#endif
    // AI: end
    ...
    // AI: below section was generated by an AI
#if defined(MASTER)
    // A dedicated MASTER build owns the Beat clock immediately.
    becomeBeatMaster();
#endif
    // AI: end
    ...
    set_wled_pattern(0, 128, 128, PatternRenderOptions(), 0, 0); // Default display
    ...
    // AI: below section was generated by an AI
#if defined(MASTER)
    // MASTER listens for tempo at boot, while its visual overlay remains off.
    sound.setTempoTracking(true);
#endif
    // AI: end
  ...
  void do_pattern_changes() {
    ...
    if (phrase >= next_state.palette_phrase) {
      ...
      if (!currentPaletteHoldPhrases) {
        ...
        nextPaletteRaw = activePaletteRaw;
        nextPaletteRawValid = activePaletteRawValid;
        nextPaletteSourceId = activePaletteSourceId;
  ...
  void set_pattern_override(uint8_t value, uint8_t auto_mode) {
    ...
    if (value) {
      ...
      // AI: below section was generated by an AI
      // Restore the complete authoritative program so NOBLEND cannot retain a
      // manually selected palette or lose its table-defined render geometry.
      if (currentPatternProgramValid) {
        set_wled_pattern(currentPatternProgram, current_state.palette_id);
      } else {
        const PatternDef& definition = gPatterns[current_state.pattern_id];
        uint8_t param = modeParameter(auto_mode);
        uint8_t speed = definition.renderDefaults.speed
            ? definition.renderDefaults.speed
            : param;
        uint8_t intensity = definition.renderDefaults.intensity
            ? definition.renderDefaults.intensity
            : param;
        set_wled_pattern(
            auto_mode,
            speed,
            ...
            definition.renderDefaults.paletteBlend);
      }
      // AI: end
    }
  }
  ...
  void activatePendingV3Palette() {
    ...
    if (v3PendingPaletteSchedule.paletteFormat == V3Palette16
      ...
      memcpy(definition.rgb, v3PendingPaletteDefinition.rgb, sizeof(definition.rgb));
      PaletteChannelEntry entry;
      entry.palettePhrase = v3PendingPaletteSchedule.effectivePhrase;
      entry.paletteId = v3PendingPaletteSchedule.legacyPaletteId;
  ...
  BeatSoundProgramExtension currentBeatSoundProgramExtension() const {
    ...
    extension.currentChance = currentSoundProgramChance;
    extension.nextChance = nextSoundProgramChance;
    return extension;
  }
  ...
  PatternProgramEntry tablePatternProgram(
    ...
    uint8_t energyParameter = modeParameter(definition.wled_fx_id);
    program.speed = definition.renderDefaults.speed
        ? definition.renderDefaults.speed
        : energyParameter;
    program.intensity = definition.renderDefaults.intensity
        ? definition.renderDefaults.intensity
        : energyParameter;
    program.paletteBlend = definition.renderDefaults.paletteBlend;
  ...
  // Adds the deterministic blend default that was absent from deployed version-1
  // Pattern programs while preserving every field they did carry.
  static PatternProgramEntry upgradePatternProgram(
      const PatternProgramEntryV1& legacy
  ) {
    PatternProgramEntry program;
    program.effectivePhrase = legacy.effectivePhrase;
    program.holdPhrases = legacy.holdPhrases;
    program.renderer = legacy.renderer;
    program.fallbackPatternId = legacy.fallbackPatternId;
    program.syncMode = legacy.syncMode;
    program.renderId = legacy.renderId;
    program.speed = legacy.speed;
    program.intensity = legacy.intensity;
    program.custom1 = legacy.custom1;
    program.custom2 = legacy.custom2;
    program.custom3 = legacy.custom3;
    program.optionMask = legacy.optionMask;
    program.optionValues = legacy.optionValues;
    program.paletteBlend = 0;
    return program;
  }

  static PatternChannelSnapshot upgradePatternSnapshot(
      const PatternChannelSnapshotV1& legacy
  ) {
    PatternChannelSnapshot snapshot;
    snapshot.state = legacy.state;
    snapshot.transitionMs = legacy.transitionMs;
    snapshot.currentProgram = upgradePatternProgram(legacy.currentProgram);
    snapshot.nextProgram = upgradePatternProgram(legacy.nextProgram);
    return snapshot;
  }
  ...
  // Represents an existing runtime palette as sixteen exact slot stops. Loading this
  // definition through WLED recreates the same `CRGBPalette16` without color transforms.
  static void makePaletteRawFrom16(
      const PaletteGradient16& palette,
      PaletteRawDefinition& definition
  ) {
    definition = PaletteRawDefinition();
    definition.count = 16;
    for (uint8_t index = 0; index < 16; index++) {
      definition.stops[index].position = index * 17;
      definition.stops[index].red = palette.rgb[index][0];
      definition.stops[index].green = palette.rgb[index][1];
      definition.stops[index].blue = palette.rgb[index][2];
    }
  }

  // Copies built-in gradient stops before WLED quantizes them. Palettes that are already
  // defined as sixteen runtime colors use exact slot stops as a lossless fallback.
  bool makePaletteRaw(uint8_t paletteId, PaletteRawDefinition& definition) {
    if (paletteId >= gBuiltInPaletteCount)
      return false;

    constexpr uint8_t gradientPaletteBase = DYNAMIC_PALETTE_COUNT + FASTLED_PALETTE_COUNT;
    if (paletteId >= gradientPaletteBase) {
      definition = PaletteRawDefinition();
      uint8_t gradientIndex = paletteId - gradientPaletteBase;
      const uint8_t* source = reinterpret_cast<const uint8_t*>(
          pgm_read_dword(&(gGradientPalettes[gradientIndex])));
      for (uint8_t index = 0; index < PALETTE_RAW_MAX_STOPS; index++) {
        memcpy_P(&definition.stops[index],
                 source + size_t(index) * sizeof(PaletteGradientStop),
                 sizeof(PaletteGradientStop));
        definition.count++;
        if (definition.stops[index].position == 255)
          return isValidPaletteRawDefinition(definition);
      }
      return false;
    }

    PaletteGradient16 palette;
    if (!makePalette16(paletteId, palette))
      return false;
    makePaletteRawFrom16(palette, definition);
    return true;
  }

  // Replays an unmodified stop table through the same loader used for local built-ins.
  // Validation guarantees the loader finds its terminator inside the bounded buffer.
  static bool expandPaletteRaw(
      const PaletteRawDefinition& definition,
      PaletteGradient16& palette
  ) {
    if (!isValidPaletteRawDefinition(definition))
      return false;

    uint8_t bytes[PALETTE_RAW_MAX_STOPS * sizeof(PaletteGradientStop)] = {0};
    size_t byteCount = size_t(definition.count) * sizeof(PaletteGradientStop);
    memcpy(bytes, definition.stops, byteCount);
    CRGBPalette16 expanded;
    expanded.loadDynamicGradientPalette(bytes);
    for (uint8_t index = 0; index < 16; index++) {
      palette.rgb[index][0] = expanded[index].r;
      palette.rgb[index][1] = expanded[index].g;
      palette.rgb[index][2] = expanded[index].b;
    }
    return true;
  }

  // Decodes both source tables through the same path used by received Palette messages.
  static bool decodePaletteRawSnapshot(
      const TubesChannelMessageV2& message,
      PaletteRawChannelSnapshot& snapshot,
      PaletteGradient16& current,
      PaletteGradient16& next
  ) {
    return readChannelMessageV2Body(message, snapshot)
        && expandPaletteRaw(snapshot.currentDefinition, current)
        && expandPaletteRaw(snapshot.nextDefinition, next);
  }
  ...
  bool installPalette16(
      ...
      bool redraw,
      const PaletteRawDefinition* rawDefinition = nullptr,
      uint8_t sourcePaletteId = PALETTE_PREDEFINED_ID_NONE
  ) {
    ...
    if (rawDefinition) {
      activePaletteRaw = *rawDefinition;
      activePaletteRawValid = true;
    } else {
      makePaletteRawFrom16(definition, activePaletteRaw);
      activePaletteRawValid = true;
    }
    activePaletteSourceId = sourcePaletteId;
  ...
  // Carries source stops when available, while literal runtime palettes use sixteen
  // exact slot stops so every palette has one bounded lossless wire representation.
  bool makePaletteRawChannelSnapshot(PaletteRawChannelSnapshot& snapshot) {
    PaletteChannelState localState = currentPaletteChannelState();
    snapshot.state.current.palettePhrase = localState.current.palettePhrase;
    snapshot.state.next.palettePhrase = localState.next.palettePhrase;
    snapshot.transitionMs = transitionDelay;
    snapshot.currentHoldPhrases = currentPaletteHoldPhrases;
    snapshot.nextHoldPhrases = nextPaletteHoldPhrases;

    if (activePaletteRawValid) {
      snapshot.currentDefinition = activePaletteRaw;
      snapshot.state.current.paletteId = activePaletteSourceId;
    } else if (!makePaletteRaw(localState.current.paletteId, snapshot.currentDefinition)) {
      return false;
    } else {
      snapshot.state.current.paletteId = localState.current.paletteId;
    }

    if (nextPaletteRawValid
        && nextPalette16Phrase == localState.next.palettePhrase
        && nextPalette16Fallback == localState.next.paletteId) {
      snapshot.nextDefinition = nextPaletteRaw;
      snapshot.state.next.paletteId = nextPaletteSourceId;
    } else if (!makePaletteRaw(localState.next.paletteId, snapshot.nextDefinition)) {
      return false;
    } else {
      snapshot.state.next.paletteId = localState.next.paletteId;
    }
    return true;
  }
  ...
  bool publishApplicationChannel(uint8_t channel) {
    ...
    TubesChannelMessageV2 message;
    auto sendPrepared = [this, channel, nowMs](TubesChannelMessageV2& prepared) {
      if (!node.isFollowing()) {
        if (!channelWinners.acceptRequest(channel, prepared.envelope, nowMs))
          return false;
        prepared.envelope.messageKind = ChannelDeclaration;
      }
      return node.sendV3ChannelV2(channel, prepared);
    };

    if (channel == PaletteChannel) {
      PaletteRawChannelSnapshot raw;
      bool prepared = makePaletteRawChannelSnapshot(raw)
          && prepareChannelMessageV2(channel, ChannelRequest, raw, message);
      bool sent = prepared && sendPrepared(message);
      if (node.serialTraceEnabled()) {
        Serial.printf(
            "PALETTE_RAW_TX body=%u current=%u/%u next=%u/%u sent=%u\n",
            prepared ? message.envelope.bodyLength : 0,
            raw.state.current.paletteId,
            raw.currentDefinition.count,
            raw.state.next.paletteId,
            raw.nextDefinition.count,
            sent);
      }
      return sent;
    }
    ...
    bool sent = sendPrepared(message);
  ...
  void update()
  {
    ...
    if (fleetPropagationTransportSuspended && updater.status == Failed) {
      if (!fleetPropagationRestoreStarted) {
        fleetPropagationRestoreStarted = restoreMeshRadioAfterDig2Go();
        if (fleetPropagationRestoreStarted)
          Serial.println(F("FLEET_OTA mesh_restore_requested_after_failure"));
      } else if (meshRadioStartedAfterDig2Go()) {
        fleetPropagationTransportSuspended = false;
        fleetPropagationRestoreStarted = false;
  ...
  void handleOverlayDraw() {
    ...
    // AI: below section was generated by an AI
    // Physical masters compact art into half a strip. TubeOS needs the complete logical
    // framebuffer because its AMOLED preview already supplies the master beat marker.
...
#if TUBES_ENABLE_DIG2GO_PEER_PROPAGATION
    if (dig2GoPeerPropagationOverlayStatus != Idle) {
      CRGB color = CRGB::Black;
      switch (dig2GoPeerPropagationOverlayStatus) {
  ...
  void load_pattern(
    ...
    if (node.serialTraceEnabled()) {
      Serial.printf(
          "PATTERN_PROGRAM applied renderer=%u id=%u fallback=%u sync=%u speed=%u intensity=%u c=%u/%u/%u options=%u/%u paletteBlend=%u hold=%u\n",
          program.renderer,
          program.renderId,
  ...
  void load_palette(TubeState &tube_state) {
    ...
    if (nextPalette16Valid
      ...
      PaletteRawDefinition sourceStops;
      const PaletteRawDefinition* source = nullptr;
      PaletteGradient16 installed = nextPalette16;
      if (nextPaletteRawValid && expandPaletteRaw(nextPaletteRaw, installed))
        source = &nextPaletteRaw;
      else if (makePaletteRaw(entry.paletteId, sourceStops)
               && expandPaletteRaw(sourceStops, installed))
        source = &sourceStops;
      installPalette16(installed, entry, true, source, nextPaletteSourceId);
      nextPalette16Valid = false;
      nextPaletteRawValid = false;
      nextPaletteSourceId = PALETTE_PREDEFINED_ID_NONE;
      return;
    }
    PaletteRawDefinition sourceStops;
    PaletteGradient16 definition;
    if (makePaletteRaw(entry.paletteId, sourceStops)
        && expandPaletteRaw(sourceStops, definition)) {
      installPalette16(definition, entry, true, &sourceStops, entry.paletteId);
      return;
    }
    if (makePalette16(entry.paletteId, definition)) {
      installPalette16(definition, entry, true, nullptr, entry.paletteId);
      return;
    }
    ...
    activePaletteRawValid = false;
    activePaletteSourceId = PALETTE_PREDEFINED_ID_NONE;
    current_state.palette_phrase = entry.palettePhrase;
    current_state.palette_id = entry.paletteId;
    if (changed)
      update_background();
  ...
  void traceSoundProgramSchedule() const {
    ...
    Serial.printf(
        ...
        nextSoundProgramChance,
        nextSoundProgram.effectivePhrase
    );
  }

  SoundProgramEntry soundProgramFromDefinition(
      const SoundProgramDefinition& definition,
      uint16_t effectivePhrase
  ) const {
    SoundProgramEntry program;
    program.effectivePhrase = effectivePhrase;
  ...
  SoundProgramEntry pickSoundProgram(uint16_t effectivePhrase, uint8_t& chance) const {
    ...
    chance = gSoundPrograms[candidate].chance;
    return soundProgramFromDefinition(gSoundPrograms[candidate], effectivePhrase);
  }

  // Builds a wire-ready program from a known candidate or safe defaults for any raw WLED effect.
  SoundProgramEntry soundProgramForEffect(
      uint8_t effectId,
      uint16_t effectivePhrase,
      uint8_t& chance
  ) const {
  ...
  void refreshSoundProgramLayer() {
    ...
    sound.setOverlayChance(currentSoundProgramChance);
  ...
  void configureSoundProgram(bool enabled) {
    ...
    if (enabled) {
      nextSoundProgram = pickSoundProgram(boundary, nextSoundProgramChance);
    } else {
      nextSoundProgram = disabledSoundProgram(boundary);
      nextSoundProgramChance = SOUND_CHANCE_ALL;
    }
    traceSoundProgramSchedule();
    publishApplicationChannel(BeatChannel);
  }
  ...
  void configureSoundProgramEffect(uint16_t effectId) {
    ...
    uint8_t chance;
    SoundProgramEntry program = soundProgramForEffect(
        uint8_t(effectId), nextPhraseBoundary(), chance);
    configureSoundProgramEntry(program, chance);
  }

  // Schedules a complete serial-supplied segment program on the Beat channel.
  void configureSoundProgramEntry(SoundProgramEntry program, uint8_t chance) {
    if (!channelWinners.localMayRequest(
          BeatChannel,
          localChannelId(BeatChannel),
          node.header.id,
    ...
    nextSoundProgramChance = chance;
    soundProgramRequested = false;
    traceSoundProgramSchedule();
    publishApplicationChannel(BeatChannel);
    Serial.printf(
        "SOUND_PROGRAM scheduled phrase=%u fx=%u pal=%u speed=%u intensity=%u c=%u/%u/%u checks=%u blend=%u opacity=%u chance=%u\n",
  ...
  // Parses the complete O command; legacy ten-field commands default to full chance.
  bool parseSoundProgramEntry(
      const char* arguments,
      SoundProgramEntry& program,
      uint8_t& chance
  ) const {
    static constexpr uint8_t MIN_VALUE_COUNT = 10;
    static constexpr uint8_t MAX_VALUE_COUNT = 11;
    uint8_t values[MAX_VALUE_COUNT] = {0};
    const char* cursor = arguments;
    uint8_t count = 0;
    bool terminated = false;
    while (count < MAX_VALUE_COUNT) {
    ...
    if (count < MIN_VALUE_COUNT || !terminated)
      return false;
    ...
    chance = count == MAX_VALUE_COUNT ? values[10] : SOUND_CHANCE_ALL;
    return true;
  }
  ...
  // WLED programs accept the deployed 11 values or a twelfth palette-blend value.
  bool parsePatternProgramEntry(const char* arguments, PatternProgramEntry& program) const {
    ...
    if (arguments[0] == 'W') {
      static constexpr uint8_t LEGACY_VALUE_COUNT = 11;
      static constexpr uint8_t VALUE_COUNT = 12;
      uint8_t valueCount = 1;
      for (const char* cursor = arguments + 2; *cursor; cursor++)
        if (*cursor == ',') valueCount++;
      if (valueCount != LEGACY_VALUE_COUNT && valueCount != VALUE_COUNT)
        return false;
      uint16_t values[VALUE_COUNT] = {0};
      if (!parseDecimalFields(arguments + 2, values, valueCount, 1024))
        return false;
      if (values[0] >= strip.getModeCount()
          || values[1] > UINT8_MAX
          || values[2] > UINT8_MAX
          ...
          || (valueCount == VALUE_COUNT && values[11] > 3)) {
        return false;
      }
      ...
      program.paletteBlend = valueCount == VALUE_COUNT ? values[11] : 0;
      return true;
    }
  ...
  void configurePatternProgram(PatternProgramEntry program) {
    ...
    Serial.printf(
        "PATTERN_PROGRAM scheduled phrase=%u hold=%u renderer=%u id=%u fallback=%u sync=%u speed=%u intensity=%u c=%u/%u/%u options=%u/%u paletteBlend=%u\n",
        program.effectivePhrase,
        program.holdPhrases,
        program.renderer,
        program.renderId,
  ...
  void scheduleAudioWorkshopProgram(uint8_t index) {
    ...
    configureSoundProgramEntry(
        workshopSoundProgram(audioWorkshopIndex), definition.chance);
  ...
  void updateSoundProgramSchedule() {
    ...
    if (nextSoundProgram.effectivePhrase == UINT16_MAX) {
      ...
      if (ownsBeat && soundProgramRequested) {
        ...
        nextSoundProgram = pickSoundProgram(
            phrase + delayPhrases, nextSoundProgramChance);
        publishApplicationChannel(BeatChannel);
        traceSoundProgramSchedule();
      }
      return;
    ...
    currentSoundProgramChance = nextSoundProgramChance;
    refreshSoundProgramLayer();
    bool ownsBeat = channelWinners.localMayRequest(
        BeatChannel,
        localChannelId(BeatChannel),
    ...
    if (ownsBeat && soundProgramRequested) {
      ...
      nextSoundProgram = pickSoundProgram(nextPhrase, nextSoundProgramChance);
      publishApplicationChannel(BeatChannel);
    } else {
  ...
  void update_background() {
    ...
    // AI: below section was generated by an AI
    // Apply the Pattern and Palette channel outputs in one WLED update. This keeps
    // NOBLEND programs from rendering against a stale segment palette.
    if (currentPatternProgramValid)
      set_wled_pattern(currentPatternProgram, background.palette_id);
    else {
      const PatternRenderOptions& renderOptions = fallback.renderOptions;
      uint8_t param = modeParameter(background.wled_fx_id);
      uint8_t speed = fallback.renderDefaults.speed
          ? fallback.renderDefaults.speed
          : param;
      uint8_t intensity = fallback.renderDefaults.intensity
          ? fallback.renderDefaults.intensity
          : param;
      set_wled_pattern(
          background.wled_fx_id,
          speed,
          intensity,
          renderOptions,
          background.palette_id,
          fallback.renderDefaults.paletteBlend);
    }
    // AI: end
    refreshSoundProgramLayer();
  }
  ...
  // AI: below section was generated by an AI
  // Resolves the Palette channel's selection to the WLED palette that is actually installed.
  uint8_t resolvedWledPaletteId(uint8_t paletteId) const {
    if (paletteOverride)
      return paletteOverride;
    if (v3RuntimePaletteActive && v3RuntimePaletteId)
      return v3RuntimePaletteId;
    return paletteId;
  }
  // AI: end
  ...
  void set_wled_palette(uint8_t palette_id) {
    ...
    seg.setPalette(resolvedWledPaletteId(palette_id));

    stateChanged = true;
    stateUpdated(CALL_MODE_DIRECT_CHANGE);
  }

  // Selects a WLED effect together with the authoritative palette and interpolation mode.
  void set_wled_pattern(
      uint8_t pattern_id,
      uint8_t speed,
      uint8_t intensity,
      const PatternRenderOptions& renderOptions,
      uint8_t paletteId,
      uint8_t paletteBlendMode
  ) {
    if (!shouldRenderTubes())
      return;
    ...
    Segment& seg = strip.getMainSegment();
    // AI: below section was generated by an AI
    // Palette selection and interpolation are committed before the mode changes.
    paletteBlend = paletteBlendMode;
    seg.setPalette(resolvedWledPaletteId(paletteId));
    // AI: end
    seg.speed = speed;
  ...
  // Applies a complete Pattern-channel WLED program against the current Palette-channel selection.
  void set_wled_pattern(const PatternProgramEntry& program, uint8_t paletteId) {
    if (!shouldRenderTubes())
      return;

    paletteBlend = program.paletteBlend;
    ...
    Segment& segment = strip.getMainSegment();
    // AI: below section was generated by an AI
    segment.setPalette(resolvedWledPaletteId(paletteId));
    // AI: end
  ...
  // Converts modern palette identity into the ID carried by the Control master's
  // generation-0 State. Literal and extended palettes use their closest legacy color.
  uint8_t legacyPaletteIdForSource(
      uint8_t sourcePaletteId,
      const PaletteGradient16& definition
  ) {
    if (sourcePaletteId < gLegacyPaletteIdLimit)
      return sourcePaletteId;
    return nearestPaletteFallback(definition);
  }
  ...
  bool schedulePalette16(
    ...
    if (fallback >= gLegacyPaletteIdLimit
        || (sourcePaletteId != PALETTE_PREDEFINED_ID_NONE
            && sourcePaletteId >= gBuiltInPaletteCount)
        || holdPhrases > 1024)
      return false;
    becomePaletteMaster();
    ...
    if (rawDefinition) {
      nextPaletteRaw = *rawDefinition;
      nextPaletteRawValid = true;
    } else {
      makePaletteRawFrom16(definition, nextPaletteRaw);
      nextPaletteRawValid = true;
    }
    nextPaletteSourceId = sourcePaletteId;
  ...
  bool scheduleCustomGradient(
    ...
    if (!expandPaletteGradient(gradient, expanded))
      return false;
    PaletteRawDefinition raw;
    raw.count = gradient.count;
    for (uint8_t index = 0; index < gradient.count; index++)
      raw.stops[index] = gradient.stops[index];
    return schedulePalette16(expanded, fallback, effectivePhrase, holdPhrases, &raw);
  }
  ...
  // from its upper bound. Send their source ID and stops so Control can project them.
  ...
  // Compares every palette slot and every index accepted by WLED's three lookup modes.
  static bool verifyDecodedPalette(
      uint8_t paletteId,
      const char* positionName,
      const CRGBPalette16& expected,
      const PaletteGradient16& decoded,
      uint32_t& lookupCount
  ) {
    CRGBPalette16 actual;
    for (uint8_t slot = 0; slot < 16; slot++) {
      ...
      if (actual[slot] != expected[slot]) {
        Serial.printf(
            "PALETTE_VERIFY_FAIL id=%u position=%s slot=%u expected=%02X%02X%02X actual=%02X%02X%02X\n",
            paletteId,
            positionName,
            slot,
            expected[slot].r,
    ...
    const TBlendType blendModes[] = {NOBLEND, LINEARBLEND, LINEARBLEND_NOWRAP};
    for (uint8_t blendIndex = 0; blendIndex < 3; blendIndex++) {
      for (uint16_t index = 0; index <= UINT8_MAX; index++) {
        uint32_t expectedColor = ColorFromPalette(
            expected, index, UINT8_MAX, blendModes[blendIndex]);
        uint32_t actualColor = ColorFromPalette(
            actual, index, UINT8_MAX, blendModes[blendIndex]);
        lookupCount++;
        if (actualColor != expectedColor) {
          Serial.printf(
              "PALETTE_VERIFY_FAIL id=%u position=%s index=%u blend=%u expected=%08lX actual=%08lX\n",
              paletteId,
              positionName,
              index,
              blendIndex,
              static_cast<unsigned long>(expectedColor),
              static_cast<unsigned long>(actualColor));
          return false;
        }
      }
    }
  ...
  // Renders two 150-pixel Palette-effect buffers at every animated shift. The native
  // buffer uses a built-in WLED palette ID; the transferred buffer uses the runtime
  // palette ID populated by the decoded message.
  static bool verifyNoBlendPaletteEffectAB(
      uint8_t paletteId,
      const CRGBPalette16& nativePalette,
      const CRGBPalette16& transferredPalette,
      uint32_t& pixelCount
  ) {
    constexpr uint16_t PIXEL_COUNT = 150;
    constexpr uint8_t PALETTE_SIZE = 144; // Two complete repetitions in Palette effect 65.
    uint32_t nativePixels[PIXEL_COUNT];
    uint32_t transferredPixels[PIXEL_COUNT];
    for (uint16_t shift = 0; shift <= UINT8_MAX; shift++) {
      for (uint16_t pixel = 0; pixel < PIXEL_COUNT; pixel++) {
        int colorIndex = uint32_t(pixel) * UINT8_MAX / (PIXEL_COUNT - 1);
        colorIndex = ((PALETTE_SIZE - 112) * colorIndex) / 16;
        colorIndex -= shift;
        uint8_t paletteIndex = colorIndex;
        nativePixels[pixel] = ColorFromPalette(
            nativePalette, paletteIndex, UINT8_MAX, NOBLEND);
        transferredPixels[pixel] = ColorFromPalette(
            transferredPalette, paletteIndex, UINT8_MAX, NOBLEND);
      }
      for (uint16_t pixel = 0; pixel < PIXEL_COUNT; pixel++) {
        pixelCount++;
        if (nativePixels[pixel] != transferredPixels[pixel]) {
          Serial.printf(
              "PALETTE_AB_FAIL id=%u shift=%u pixel=%u expected=%08lX actual=%08lX\n",
              paletteId,
              shift,
              pixel,
              static_cast<unsigned long>(nativePixels[pixel]),
              static_cast<unsigned long>(transferredPixels[pixel]));
          return false;
        }
      }
    }
  ...
  // Exercises the production encoder, exact wire length, ingress validator, decoder,
  // FastLED gradient loader, and WLED palette lookup for every built-in palette.
  void verifyBuiltInPaletteRoundTrips() {
    uint16_t passed = 0;
    uint16_t failed = 0;
    uint32_t lookupCount = 0;
    uint32_t abPixelCount = 0;
    CRGBPalette16* runtimePalette = runtimeV3Palette();
    CRGBPalette16 savedRuntimePalette;
    bool savedRuntimeActive = v3RuntimePaletteActive;
    uint8_t savedPaletteOverride = paletteOverride;
    uint8_t savedPatternOverride = patternOverride;
    if (runtimePalette)
      savedRuntimePalette = *runtimePalette;
    v3RuntimePaletteActive = false;
    paletteOverride = 0;
    patternOverride = 0;
    uint8_t demonstrationPattern = preferredPatternForWledEffect(FX_MODE_PALETTE);
    PatternProgramEntry noBlendProgram = tablePatternProgram(
        demonstrationPattern, currentPhrase(), 0);
    Serial.printf(
        "PALETTE_VERIFY_START palettes=%u body=%u wire=%u abSegments=2 abPixels=150 pattern=%u effect=%u blend=%u\n",
        gBuiltInPaletteCount,
        sizeof(PaletteRawChannelSnapshot),
        channelMessageV2WireSize(sizeof(PaletteRawChannelSnapshot)),
        demonstrationPattern,
        noBlendProgram.renderId,
        noBlendProgram.paletteBlend);

    for (uint16_t currentId = 0; currentId < gBuiltInPaletteCount; currentId++) {
      uint8_t nextId = uint8_t((currentId + 1) % gBuiltInPaletteCount);
      CRGBPalette16 expectedCurrent;
      CRGBPalette16 expectedNext;
      strip.getMainSegment().copyPalette(expectedCurrent, currentId);
      strip.getMainSegment().copyPalette(expectedNext, nextId);

      PaletteRawChannelSnapshot sentSnapshot;
      sentSnapshot.state.current.palettePhrase = 0;
      sentSnapshot.state.current.paletteId = currentId;
      sentSnapshot.state.next.palettePhrase = UINT16_MAX;
      sentSnapshot.state.next.paletteId = nextId;
      sentSnapshot.currentHoldPhrases = 0;
      sentSnapshot.nextHoldPhrases = 0;
      bool prepared = makePaletteRaw(
          currentId, sentSnapshot.currentDefinition)
          && makePaletteRaw(nextId, sentSnapshot.nextDefinition);

      TubesChannelMessageV2 sentMessage;
      prepared = prepared && prepareChannelMessageV2(
          PaletteChannel, ChannelDeclaration, sentSnapshot, sentMessage);
      sentMessage.header = node.header;
      sentMessage.header.version = TUBES_PROTOCOL_V3;
      sentMessage.recipients = RECIPIENTS_ALL;
      size_t wireLength = channelMessageV2WireSize(sentMessage.envelope.bodyLength);

      TubesChannelMessageV2 receivedMessage;
      if (prepared)
        memcpy(&receivedMessage, &sentMessage, wireLength);
      PaletteRawChannelSnapshot receivedSnapshot;
      PaletteGradient16 decodedCurrent;
      PaletteGradient16 decodedNext;
      CRGBPalette16 transferredPalette;
      bool decoded = prepared
          && isValidChannelMessageV2Prefix(
              reinterpret_cast<const uint8_t*>(&receivedMessage), wireLength)
          && isValidV3ChannelV2(
              PaletteChannel, receivedMessage, sentMessage.header, RECIPIENTS_ALL)
          && decodePaletteRawSnapshot(
              receivedMessage, receivedSnapshot, decodedCurrent, decodedNext)
          && memcmp(&sentSnapshot, &receivedSnapshot, sizeof(sentSnapshot)) == 0
          && receivedSnapshot.state.current.paletteId == currentId
          && receivedSnapshot.state.next.paletteId == nextId;
      bool selected = decoded
          && runtimePalette
          && installRuntimeV3Palette(decodedCurrent.rgb);
      v3RuntimePaletteActive = selected;
      if (selected)
        set_wled_pattern(noBlendProgram, currentId);
      Segment& segment = strip.getMainSegment();
      bool productionApplied = selected
          && noBlendProgram.renderId == FX_MODE_PALETTE
          && noBlendProgram.paletteBlend == PATTERN_PALETTE_NOBLEND
          && segment.mode == FX_MODE_PALETTE
          && segment.palette == v3RuntimePaletteId
          && paletteBlend == PATTERN_PALETTE_NOBLEND;
      if (selected)
        segment.copyPalette(transferredPalette, v3RuntimePaletteId);
      bool exact = decoded
          && verifyDecodedPalette(
              currentId, "current", expectedCurrent, decodedCurrent, lookupCount)
          && verifyDecodedPalette(
              nextId, "next", expectedNext, decodedNext, lookupCount)
          && productionApplied
          && verifyNoBlendPaletteEffectAB(
              currentId, expectedCurrent, transferredPalette, abPixelCount);
      if (!exact) {
        failed++;
        if (!decoded || !selected) {
          Serial.printf(
              "PALETTE_VERIFY_FAIL id=%u next=%u stage=%s prepared=%u body=%u wire=%u runtimeId=%u\n",
              currentId,
              nextId,
              decoded && selected ? "noblend_program_binding" :
                  (decoded ? "runtime_palette_selection" : "wire_or_decode"),
              prepared,
              sentMessage.envelope.bodyLength,
              wireLength,
              v3RuntimePaletteId);
        }
      } else {
        passed++;
        Serial.printf(
            "PALETTE_VERIFY_PASS current=%u/%u next=%u/%u runtimeId=%u blend=%u abPixels=38400\n",
            currentId,
            sentSnapshot.currentDefinition.count,
            nextId,
            sentSnapshot.nextDefinition.count,
            v3RuntimePaletteId,
            paletteBlend);
      }
      delay(1);
    ...
    if (runtimePalette)
      *runtimePalette = savedRuntimePalette;
    v3RuntimePaletteActive = savedRuntimeActive;
    paletteOverride = savedPaletteOverride;
    patternOverride = savedPatternOverride;
    update_background();
    Serial.printf(
        "PALETTE_VERIFY_COMPLETE passed=%u failed=%u lookups=%lu abPixels=%lu pattern=%u effect=%u blend=%u\n",
        passed,
        failed,
        static_cast<unsigned long>(lookupCount),
        static_cast<unsigned long>(abPixelCount),
        demonstrationPattern,
        noBlendProgram.renderId,
        noBlendProgram.paletteBlend);
  }
  ...
  void keyboard_command(char *command) {
    ...
    // AI: below section was generated by an AI
    if (key == 'v' && !command[1]) {
      verifyBuiltInPaletteRoundTrips();
      return;
    }
    // AI: end
    // AI: below section was generated by an AI
    ...
    if (key == 'c' && strchr(command + 1, ',')) {
      ...
      if (!parseDecimalFields(command + 1, values, 2, 1024)
          ...
          || !makePaletteRaw(uint8_t(values[0]), raw)) {
      ...
      if (!schedulePalette16(
              definition, fallback, nextPhraseBoundary(), values[1], &raw,
              uint8_t(values[0]))) {
    ...
    if (key == 'O' && strchr(command + 1, ',')) {
      SoundProgramEntry program;
      uint8_t chance;
      if (!parseSoundProgramEntry(command + 1, program, chance)) {
        Serial.println(F("SOUND_PROGRAM rejected: expected 10 or 11 comma-separated values"));
        return;
      }
      configureSoundProgramEntry(program, chance);
      return;
  ...
  // AI: below section was generated by an AI
  // Schedules a curated TubeOS choice using the existing additive pattern-program path.
  bool selectS3Pattern(uint8_t patternId) {
    if (node.isFollowing() || patternId >= gPatternCount)
      return false;
    uint16_t holdPhrases = randomPatternDuration(gPatterns[patternId]);
    PatternProgramEntry program = tablePatternProgram(
        patternId, nextPhraseBoundary(), holdPhrases);
    program.syncMode = All;
    configurePatternProgram(program);
    return true;
  }
  ...
  bool scheduleS3Gradient(const uint32_t colors[3]) {
    ...
    if (!expandPaletteGradient(gradient, expanded))
      return false;
    const uint8_t fallback = nearestPaletteFallback(gradient);
    const uint16_t effectivePhrase = nextPhraseBoundary();
    scheduleCustomGradient(
        gradient,
        fallback,
        effectivePhrase,
        random8(MIN_COLOR_CHANGE_PHRASES, MAX_COLOR_CHANGE_PHRASES));
    // The local schedule is authoritative even when this immediate radio send has no recipient.
  ...
  // Maps the fixed TubeOS choices back from the complete Beat-channel program.
  uint8_t s3BeatOverlayChoice(bool next) const {
    const SoundProgramEntry& program = next ? nextSoundProgram : currentSoundProgram;
    if (!soundProgramEnabled(program)) return 0;
    for (uint8_t index = 0; index < S3_BEAT_OVERLAY_COUNT; index++) {
      if (program.wledEffect == gSoundPrograms[index].wledEffect)
        return index + 1;
    }
    return 0;
  }

  bool setS3TempoListening(bool enabled) {
    autoTempoClockLocked = false;
    sound.setTempoTracking(enabled);
    return sound.tempoTracking == enabled;
  }

  // Makes this fixed MASTER the Beat owner and restarts the shared 16-beat phrase.
  bool tapS3Downbeat() {
    if (node.isFollowing()) return false;
    becomeBeatMaster();
    beats.start_phrase();
  ...
  // Queues one curated audio overlay, or Off, at the next phrase boundary.
  bool selectS3BeatOverlay(uint8_t choice) {
    if (node.isFollowing() || choice > S3_BEAT_OVERLAY_COUNT) return false;
    becomeBeatMaster();
    const uint16_t boundary = nextPhraseBoundary();
    soundProgramRequested = false;
    if (choice == 0) {
      nextSoundProgram = disabledSoundProgram(boundary);
      nextSoundProgramChance = SOUND_CHANCE_ALL;
    } else {
      const SoundProgramDefinition& definition = gSoundPrograms[choice - 1];
      nextSoundProgram = soundProgramFromDefinition(definition, boundary);
      nextSoundProgramChance = definition.chance;
    }
    traceSoundProgramSchedule();
    publishApplicationChannel(BeatChannel);
    return s3BeatOverlayChoice(true) == choice;
  }
  // AI: end
  ...
  bool isValidPatternChannelSnapshot(const PatternChannelSnapshot& snapshot) const {
    return snapshot.magic == PATTERN_PROGRAM_MAGIC
        && snapshot.version == PATTERN_PROGRAM_VERSION_2
  ...
  bool isValidPatternChannelSnapshot(
      const PatternChannelSnapshotV1& snapshot
  ) const {
    return snapshot.magic == PATTERN_PROGRAM_MAGIC
        && snapshot.version == PATTERN_PROGRAM_VERSION_1
        && snapshot.length == sizeof(PatternProgramEntryV1) * 2
        && snapshot.reserved[0] == 0
        && snapshot.reserved[1] == 0
        && isValidPatternChannelState(snapshot.state)
        && isValidPatternProgramEntry(
            upgradePatternProgram(snapshot.currentProgram), snapshot.state.current)
        && isValidPatternProgramEntry(
            upgradePatternProgram(snapshot.nextProgram), snapshot.state.next);
  }

  static bool isValidPalettePhraseSchedule(const PaletteChannelState& state) {
  ...
  bool isValidPaletteChannelState(const PaletteChannelState& state) const {
    return state.current.paletteId < gLegacyPaletteIdLimit
        && state.next.paletteId < gLegacyPaletteIdLimit
        && isValidPalettePhraseSchedule(state);
  }

  bool isValidRawPaletteChannelState(const PaletteChannelState& state) const {
    bool currentIdValid = state.current.paletteId == PALETTE_PREDEFINED_ID_NONE
        || state.current.paletteId < gBuiltInPaletteCount;
    bool nextIdValid = state.next.paletteId == PALETTE_PREDEFINED_ID_NONE
        || state.next.paletteId < gBuiltInPaletteCount;
    return currentIdValid && nextIdValid && isValidPalettePhraseSchedule(state);
  }
  ...
  virtual bool isValidV3ChannelV2(
    ...
    if (channel == BeatChannel) {
      ...
      return readChannelMessageV2Body(message, snapshot)
          ...
          && snapshot.soundProgram.version == BEAT_SOUND_PROGRAM_VERSION_2
          && snapshot.soundProgram.length == sizeof(SoundProgramEntry) * 2 + 2
    ...
    if (channel == PatternChannel) {
      ...
      if (message.envelope.bodyLength == sizeof(PatternChannelSnapshotV1)) {
        PatternChannelSnapshotV1 snapshot;
        return readChannelMessageV2Body(message, snapshot)
    ...
    if (channel == PaletteChannel) {
      if (message.envelope.bodyLength == sizeof(PaletteRawChannelSnapshot)) {
        PaletteRawChannelSnapshot snapshot;
        return readChannelMessageV2Body(message, snapshot)
            && isValidRawPaletteChannelState(snapshot.state)
            && snapshot.currentHoldPhrases <= 1024
            && snapshot.nextHoldPhrases <= 1024
            && isValidPaletteRawDefinition(snapshot.currentDefinition)
            && isValidPaletteRawDefinition(snapshot.nextDefinition);
      }
  ...
  virtual bool onV3ChannelV2(
    ...
    if (channel == BeatChannel) {
      ...
      acceptSoundProgramSchedule(
          ...
          snapshot.soundProgram.currentChance,
          snapshot.soundProgram.nextChance);
      acceptBeatTransient(
      ...
      if (message.envelope.bodyLength == sizeof(PatternChannelSnapshot)
          || message.envelope.bodyLength == sizeof(PatternChannelSnapshotV1)) {
        PatternChannelSnapshot fullSnapshot;
        if (message.envelope.bodyLength == sizeof(PatternChannelSnapshotV1)) {
          PatternChannelSnapshotV1 legacySnapshot;
          if (!readChannelMessageV2Body(message, legacySnapshot))
            return false;
          fullSnapshot = upgradePatternSnapshot(legacySnapshot);
        } else if (!readChannelMessageV2Body(message, fullSnapshot)) {
          return false;
        }
      ...
      PaletteChannelSnapshot snapshot;
      PaletteRawDefinition currentRaw;
      PaletteRawDefinition nextRaw;
      uint8_t currentSourceId = PALETTE_PREDEFINED_ID_NONE;
      uint8_t nextSourceId = PALETTE_PREDEFINED_ID_NONE;
      bool hasRawDefinitions = false;
      if (message.envelope.bodyLength == sizeof(PaletteRawChannelSnapshot)) {
        PaletteRawChannelSnapshot rawSnapshot;
        if (!decodePaletteRawSnapshot(
                message,
                rawSnapshot,
                snapshot.currentDefinition,
                snapshot.nextDefinition))
          return false;
        snapshot.state = rawSnapshot.state;
        snapshot.transitionMs = rawSnapshot.transitionMs;
        snapshot.currentHoldPhrases = rawSnapshot.currentHoldPhrases;
        snapshot.nextHoldPhrases = rawSnapshot.nextHoldPhrases;
        currentRaw = rawSnapshot.currentDefinition;
        nextRaw = rawSnapshot.nextDefinition;
        currentSourceId = rawSnapshot.state.current.paletteId;
        nextSourceId = rawSnapshot.state.next.paletteId;
        hasRawDefinitions = true;
      } else if (message.envelope.bodyLength == sizeof(PaletteChannelSnapshotV1)) {
      ...
      if (!hasRawDefinitions) {
        currentSourceId = snapshot.state.current.paletteId;
        nextSourceId = snapshot.state.next.paletteId;
      }
      ...
      if (hasRawDefinitions) {
        snapshot.state.current.paletteId = legacyPaletteIdForSource(
            currentSourceId, snapshot.currentDefinition);
        snapshot.state.next.paletteId = legacyPaletteIdForSource(
            nextSourceId, snapshot.nextDefinition);
        if (node.serialTraceEnabled()) {
          Serial.printf(
              "PALETTE_RAW_RX current=%u/%u->%u next=%u/%u->%u\n",
              currentSourceId,
              currentRaw.count,
              snapshot.state.current.paletteId,
              nextSourceId,
              nextRaw.count,
              snapshot.state.next.paletteId);
        }
      }
      ...
      installPalette16(
          ...
          true,
          hasRawDefinitions ? &currentRaw : nullptr,
          currentSourceId);
      ...
      if (hasRawDefinitions)
        nextPaletteRaw = nextRaw;
      else
        makePaletteRawFrom16(snapshot.nextDefinition, nextPaletteRaw);
      nextPaletteSourceId = nextSourceId;
      ...
      if (payload.envelope.bodyLength == sizeof(PaletteChannelSchedule)) {
        ...
        installPalette16(
            *currentDefinition,
            ...
            true,
            nullptr,
            schedule.state.current.paletteId);
        ...
        nextPalette16Valid = true;
        makePaletteRawFrom16(*followingDefinition, nextPaletteRaw);
        nextPaletteRawValid = true;
        nextPaletteSourceId = schedule.state.next.paletteId;
