...
# Resolve the ephemeral Device ID before selection so a stale shared update AP
# cannot redirect an exact-ID firmware write to another pole.
resolve_target_mac() {
  local manifest
  local node_decimal=$((16#$target_id))
  manifest="$("$python_bin" "$mesh_reporter" manifest "$serial_device" --timeout 5)"
  printf '%s\n' "$manifest" | "$jq_bin" -e -r \
    --argjson node "$node_decimal" \
    '[.[] | select(.node == $node) | .mac]
     | if length == 1 then .[0] else error("Device ID did not resolve uniquely") end'
}
# AI: end
...
upgrade_one() {
  ...
  local expected_target_mac=""
  if [[ -n "$target_id" ]]; then
    expected_target_mac="$(resolve_target_mac)"
    echo "Device ID 0x$target_id is MAC $expected_target_mac."
  fi
  select_and_connect
  enroll_selected_device
  local selected_mac="$device_mac"
  if [[ -n "$expected_target_mac" ]]; then
    require_expected_mac "$expected_target_mac"
  fi
