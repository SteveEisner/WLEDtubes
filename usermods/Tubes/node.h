...
#if TUBES_ENABLE_DIG2GO_PUSH_BRIDGE && defined(ARDUINO_ARCH_ESP32)
#include <esp_now.h>
#endif
...
class LightNode {
  ...
  protected:
    ...
    bool broadcastMessage(NodeMessage *message, bool is_rebroadcast=false) {
#if TUBES_ENABLE_DIG2GO_PUSH_BRIDGE
        ...
        const bool success = espnowBroadcast.send((const uint8_t*)message, sizeof(*message));
#ifdef NODE_DEBUGGING
        if (!success) {
  ...
  public:
    ...
    bool sendLegacyNeighborCommand(CommandId command, const void* data, uint8_t len) {
        if (len > MESSAGE_DATA_SIZE) return false;
        NodeMessage message;
        message.header = legacyHeader();
        static_assert(RECIPIENTS_NEIGHBORS == 2,
            "legacy INFO and current NEIGHBORS wire value must remain identical");
        // Deployed v13/v14 calls wire value 2 INFO: accept from any direct
        // neighbor and never relay. Current firmware calls the same value
        // NEIGHBORS. That shared one-hop meaning avoids any root/uplink race.
        message.recipients = RECIPIENTS_NEIGHBORS;
        message.command = command;
...
#if TUBES_ENABLE_DIG2GO_PUSH_BRIDGE
        if (transportSuspended) {
            if (espnowBroadcast.getState() == ESPNOWBroadcast::STARTED) {
...
#if TUBES_ENABLE_DIG2GO_PUSH_BRIDGE
    // AI: below section was generated by an AI
    // Keep the Tubes transport quiescent while STA mode is temporarily used for
    // the receiver AP. Wi-Fi's STA_START event may initialize ESP-NOW again, so
    // update() tears that instance down before any Tubes work is processed.
    void suspendTransportForStationJoin(bool suspended) {
