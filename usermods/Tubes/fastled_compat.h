...
// Preserve the FastLED 3.6 trigonometric approximations used when the Tubes patterns
// were authored. WLED's newer approximations have slightly different curves, which
// makes deterministic patterns disagree with legacy poles.
// Source: https://github.com/FastLED/FastLED/blob/3.6.0/src/lib8tion/trig8.h
inline uint8_t sin8(uint8_t theta) {
  static constexpr uint8_t baseSlope[] = {0, 49, 49, 41, 90, 27, 117, 10};
  uint8_t offset = theta;
  if (theta & 0x40) offset = uint8_t(255 - offset);
  offset &= 0x3F;
  uint8_t sectionOffset = offset & 0x0F;
  if (theta & 0x40) sectionOffset++;
  uint8_t section = offset >> 4;
  uint8_t y = ((baseSlope[section * 2 + 1] * sectionOffset) >> 4)
      + baseSlope[section * 2];
  if (theta & 0x80) y = uint8_t(-int8_t(y));
  return y + 128;
}
...
inline int16_t sin16(uint16_t theta) {
  ...
  uint8_t sectionOffset = uint8_t(offset) / 2;
  int16_t y = slope[section] * sectionOffset + base[section];
  return (theta & 0x8000) ? -y : y;
}

inline int16_t cos16(uint16_t theta) { return sin16(theta + 16384); }
// AI: end
...
inline uint8_t beatsin8(uint16_t bpm, uint8_t low = 0, uint8_t high = 255, uint32_t timebase = 0, uint8_t phase = 0) {
  uint8_t value = sin8(beat8(bpm, timebase) + phase);
  return low + scale8(value, high - low);
}
inline uint16_t beatsin16(uint16_t bpm, uint16_t low = 0, uint16_t high = 65535, uint32_t timebase = 0, uint16_t phase = 0) {
  uint16_t value = sin16(beat16(bpm, timebase) + phase) + 32768;
  return low + scale16(value, high - low);
}
