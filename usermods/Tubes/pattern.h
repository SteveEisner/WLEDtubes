...
// Optional table values override the energy-derived slider defaults. Zero keeps the
// existing energy policy; entries use overrides only when an effect needs a specific
// geometry to expose its palette well.
struct PatternRenderDefaults {
  uint8_t speed;
  uint8_t intensity;
  uint8_t paletteBlend;
};

constexpr uint8_t PATTERN_PALETTE_NOBLEND = 3;
// AI: end
...
typedef struct {
  ...
  PatternRenderDefaults renderDefaults;
} PatternDef;
...
PatternDef gPatterns[] = {
  ...
  // AI: below section was generated by an AI
  // Two complete palette repetitions make color boundaries visible on a pole.
  {FX_MODE_PALETTE, draw_wled_fx, {ShortDuration}, {0, 0},
      {90, 144, PATTERN_PALETTE_NOBLEND}}, // 65
  // AI: end
