...
class VirtualStrip {
  ...
  CRGB palette_color(uint8_t c, uint8_t offset=0, uint8_t brightness=255) const {
    ...
    // Tubes patterns were authored against pre-16 WLED palettes, whose entries were already
    // gamma-adjusted before pattern brightness was applied. Preserve that order here; the
    // completed Tubes frame is converted back to WLED's linear framebuffer at its output boundary.
    CRGB color(segment.color_from_palette(c + offset, false, true, 255, 255));
    color.r = gamma8(color.r);
    color.g = gamma8(color.g);
    color.b = gamma8(color.b);
    nscale8x3(color.r, color.g, color.b, brightness);
    return color;
    // AI: end
  }
