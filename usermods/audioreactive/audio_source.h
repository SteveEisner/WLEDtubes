...
// Allows boards whose vendor reference reserves I2S0 to select the second controller.
#ifdef I2S_USE_PORT_1
#define AUDIOREACTIVE_I2S_PORT I2S_NUM_1
#define AUDIOREACTIVE_I2S_PERIPHERAL PERIPH_I2S1_MODULE
#else
#define AUDIOREACTIVE_I2S_PORT I2S_NUM_0
#define AUDIOREACTIVE_I2S_PERIPHERAL PERIPH_I2S0_MODULE
#endif
// AI: end
...
// AI: below section was generated by an AI
// Some multichannel codecs require a complete two-slot frame even when WLED consumes mono samples.
#ifdef I2S_USE_STEREO_SLOTS
...
class I2SSource : public AudioSource {
  public:
    ...
    virtual void initialize(int8_t i2swsPin = I2S_PIN_NO_CHANGE, int8_t i2ssdPin = I2S_PIN_NO_CHANGE, int8_t i2sckPin = I2S_PIN_NO_CHANGE, int8_t mclkPin = I2S_PIN_NO_CHANGE) {
      ...
      esp_err_t err = i2s_driver_install(AUDIOREACTIVE_I2S_PORT, &_config, 0, nullptr);
      if (err != ESP_OK) {
#ifdef TUBES_S3_FIELD_OS
        Serial.printf("S3_AUDIO driver_install port=%u error=%d\n", AUDIOREACTIVE_I2S_PORT, err);
#endif
      ...
      err = i2s_set_pin(AUDIOREACTIVE_I2S_PORT, &_pinConfig);
      if (err != ESP_OK) {
#ifdef TUBES_S3_FIELD_OS
        Serial.printf("S3_AUDIO set_pin port=%u error=%d\n", AUDIOREACTIVE_I2S_PORT, err);
#endif
        DEBUGSR_PRINTF("AR: Failed to set i2s pin config: %d\n", err);
        i2s_driver_uninstall(AUDIOREACTIVE_I2S_PORT);  // uninstall already-installed driver
        return;
      }

#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(4, 2, 0)
      err = i2s_set_clk(AUDIOREACTIVE_I2S_PORT, _sampleRate, I2S_SAMPLE_RESOLUTION,
#ifdef I2S_USE_STEREO_SLOTS
                        I2S_CHANNEL_STEREO
#else
                        I2S_CHANNEL_MONO
#endif
      );  // Set bit clocks and route MCLK when needed.
      if (err != ESP_OK) {
#ifdef TUBES_S3_FIELD_OS
        Serial.printf("S3_AUDIO set_clk port=%u error=%d\n", AUDIOREACTIVE_I2S_PORT, err);
#endif
      ...
      _initialized = true;
#ifdef TUBES_S3_FIELD_OS
      Serial.printf("S3_AUDIO ready port=%u rate=%lu bits=%u slots=2 mclk=%d\n",
          AUDIOREACTIVE_I2S_PORT, static_cast<unsigned long>(_sampleRate),
          _config.bits_per_sample, _mclkPin);
#endif
    }

    virtual void deinitialize() {
      _initialized = false;
      esp_err_t err = i2s_driver_uninstall(AUDIOREACTIVE_I2S_PORT);
      if (err != ESP_OK) {
    ...
    virtual void getSamples(FFTsampleType *buffer, uint16_t num_samples) {
      if (_initialized) {
        ...
        err = i2s_read(AUDIOREACTIVE_I2S_PORT, (void *)newSamples, sizeof(newSamples), &bytes_read, portMAX_DELAY);
        if (err != ESP_OK) {
