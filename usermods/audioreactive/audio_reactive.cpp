...
// Consumers use this completion counter to avoid reprocessing one FFT result.
static volatile uint32_t fftResultFrame = 0;
static volatile uint32_t fftResultTimestampUs = 0;
static AudioTempoData tempoSpectrumData;
// AI: end
...
// AI: below section was generated by an AI
// Publishes the new FFT mapping before WLED modifies it for visual effects.
static void publishTempoSpectrum(bool noiseGateOpen) {
  uint32_t sequence = __atomic_load_n(&tempoSpectrumData.publishedSequence, __ATOMIC_RELAXED) + 1;
  AudioTempoSpectrum& destination = tempoSpectrumData.slots[sequence & 1U];
  destination.timestampUs = micros();
  for (uint8_t bin = 0; bin < NUM_GEQ_CHANNELS; bin++)
    destination.bands[bin] = noiseGateOpen ? fftCalc[bin] : 0.0f;
  __atomic_store_n(&tempoSpectrumData.publishedSequence, sequence, __ATOMIC_RELEASE);
}
// AI: end
#ifdef SR_DEBUG
...
// AI: below section was generated by an AI
#ifndef UM_AUDIOREACTIVE_SAMPLE_RATE
#define UM_AUDIOREACTIVE_SAMPLE_RATE 22050
#endif
constexpr SRate_t SAMPLE_RATE = UM_AUDIOREACTIVE_SAMPLE_RATE;
// AI: end
...
void FFTcode(void * parameter)
{
  ...
  for(;;) {
    ...
    */                                    //  Range
    ...
    // AI: below section was generated by an AI
    publishTempoSpectrum(fabsf(sampleAvg) > 0.5f);
    // AI: end
...
static void postProcessFFTResults(bool noiseGateOpen, int numberOfChannels) // post-processing and post-amp of GEQ channels
{
    ...
    // AI: below section was generated by an AI
    // Publishing after every band is stored makes the preceding frame complete.
    fftResultTimestampUs = micros();
    fftResultFrame++;
    // AI: end
}
////////////////////
  ...
  public:
    ...
    void setup() override
    {
      ...
      if (!initDone) {
        ...
        // AI: below section was generated by an AI
        um_data->u_data[8] = (void*)&fftResultFrame;
        um_data->u_type[8] = UMT_UINT32;
        um_data->u_data[9] = (void*)&fftResultTimestampUs;
        um_data->u_type[9] = UMT_UINT32;
        um_data->u_data[10] = &tempoSpectrumData;
        um_data->u_type[10] = UMT_BYTE_ARR;
        // AI: end
      }
    ...
    // AI: below section was generated by an AI
    // Changes audio processing without persisting configuration. Disabling suspends
    // the FFT task; enabling resumes it after setup has initialized the audio source.
    bool setEnabled(bool requestedEnabled) override
    {
      if (enabled == requestedEnabled) return true;
      ...
      if (addPalettes) {
        if (wasEnabled && !enabled) removeAudioPalettes();
        if (!wasEnabled && enabled) createAudioPalettes();
      }
      return true;
    }
    // AI: end
    ...
    void readFromJsonState(JsonObject& root) override
    {
      ...
      if (!usermod.isNull()) {
        if (usermod[FPSTR(_enabled)].is<bool>()) {
          // AI: below section was generated by an AI
          setEnabled(usermod[FPSTR(_enabled)].as<bool>());
          // AI: end
        }
#ifdef ARDUINO_ARCH_ESP32
